...
function Profile() {
  ...
  useEffect(() => {
    const fetchProfileInfo = async () => {
      try {
        ...
        const vehicleResponse = await axios.get('http://127.0.0.1:8000/api/view-vehicles/', {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
          }
  ...
  const addVehicle = async (vehicleData) => {
    try {
      ...
      const response = await axios.post('http://127.0.0.1:8000/api/add-vehicle/', vehicleData, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
  ...
  const handleDeleteVehicle = async () => {
    try {
        ...
        await axios.delete('http://127.0.0.1:8000/api/delete-vehicle/', {
            headers: {
                Authorization: `Bearer ${token}`,
                'Content-Type': 'application/json',
            },
